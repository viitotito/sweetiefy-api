...
for (const varName of requiredEnvVars) {
  if (!process.env[varName]) {
    console.error(
      `❌ Erro: A variável de ambiente ${varName} não está definida.`
    );
    process.exit(1);
  }
}
...
async function deleteUploads() {
  ...
  try {
    const stats = await fs.stat(uploadsPath).catch(() => null);
    if (!stats || !stats.isDirectory()) {
      console.log(`- Pasta "uploads" não existe — nada para remover.`);
      return;
    }

    console.log(`- Limpando arquivos da pasta "uploads"...`);
    ...
    console.log(`- Arquivos da pasta "uploads" removidos com sucesso.`);
  } catch (error) {
    console.error(`❌ Erro ao tentar limpar a pasta uploads:`, error);
  }
}
...
async function applySchema() {
  ...
  try {
    ...
    console.error(
      `❌ Erro fatal: Não foi possível ler o arquivo de schema em ${sqlFilePath}.`
    );
    throw error;
  }
