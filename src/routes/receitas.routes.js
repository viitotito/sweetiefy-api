...
router.get("/:id", async (req, res) => {
    const receita_id = Number(req.params.id);

    if (!Number.isInteger(receita_id) || receita_id <= 0) {
    ...
    try {
        ...
        console.error("Erro ao buscar receita e ingredientes:", e);
        res.status(500).json({ erro: "Erro interno do servidor." });
    }
});
...
router.post("/", async (req, res) => {
    ...
    if (!nome || !uid || precoNum < 0) {
        return res.status(400).json({ erro: "Dados de receita incompletos ou inválidos." });
    }
...
router.put("/:id", async (req, res) => {
    ...
    if (!temNomeValido || !precoValido || !temUidValido) {
        return res.status(400).json({
            erro: "Para PUT, envie todos os campos: nome (string), preco (numero), e usuario_id (inteiro>0)",
        });
    }
...
router.patch("/:id", async (req, res) => {
    ...
    if (nome === undefined && descricao === undefined && imagem_url === undefined && preco === undefined && usuario_id === undefined) {
...
router.delete("/:receitaId/ingredientes/:ingredienteId", async (req, res) => {
    ...
    if (!Number.isInteger(receita_id) || receita_id <= 0 || !Number.isInteger(ingrediente_id) || ingrediente_id <= 0) {
